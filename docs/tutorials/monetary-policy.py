...
#
# The $\Sigma_u$ matrix allows us to capture correlation of residuals across equations. Consequently, the off-diagonal elements of $\Sigma_u$ tell us how any two series covary. However, they do not tell us which caused which and it is precisely this ambiguity that is addressed in the identification problem.
#
...
#
# We now have posterior draws of the reduced-form parameters: the coefficient matrices $A_1, \ldots, A_p$ and the residual covariance $\Sigma_u$. The reduced-form residuals $u_t$ are correlated across equations, but we want to recover the *structural* shocks $\varepsilon_t$. Unlike the reduced-form residuals, these structural shocks are orthogonal latent variables from which we can elicit a causal interpretation.
#
# Throughout this notebook, the term _"structural"_ is used to mean _"given a causal interpretation by the identifying assumptions."_. The structural model assumes
#
# $$
...
#
# This is the *price puzzle*. Higher interest rates reduce demand, which should put downward pressure on prices, yet the estimated response goes the wrong way. Why does this happen? The most common explanation: the Fed raises rates when it expects future inflation. A three-variable VAR cannot capture the full set of data and signals the Fed uses when making decisions, so the "surprise" component of the rate change still contains a predictable response to anticipated inflation. The subsequent price increase is the inflation the Fed saw coming rather than an effect of the rate hike. The "shock" is not really a shock.
#
...
#
# Considering the prices (middle panel), the CPI response is negative by construction, hovering around -0.15 to -0.20. There is no price puzzle here because we *imposed* that prices cannot rise. The absence of the puzzle is an assumption rather than a finding.
#
...
#
# Compare these bands with the Cholesky IRFs above. The sign restriction bands are much wider, especially for output: the data are consistent with many different structural decompositions, and sign restrictions honestly report this.
#
