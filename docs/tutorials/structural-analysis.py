...
def _():
    ...
    return Cholesky, NUTSSampler, VAR, VARData, np, pd, plt, select_lag_order
...
def _(df, np, pd):
    ...
    for _col in df.columns:
        ...
        climatology[_col] = fitted
        anomalies[_col] = y - fitted
...
@app.cell
def _(anomalies, climatology, df, plt):
    _fig, axes = plt.subplots(4, 2, figsize=(10, 7), sharex="col")
    units = {"pressure": "hPa", "wind": "m/s", "temperature": "C", "humidity": "%"}
    for i, _col in enumerate(df.columns):
        axes[i, 0].plot(df.index, df[_col], linewidth=0.4, color="0.4", label="observed")
        axes[i, 0].plot(df.index, climatology[_col], color="C3", linewidth=1.5, label="climatology")
        axes[i, 0].set_ylabel(f"{_col} ({units[_col]})")
        if i == 0:
            axes[i, 0].legend(fontsize=8)
            axes[i, 0].set_title("Raw + seasonal fit")
        axes[i, 1].plot(df.index, anomalies[_col], linewidth=0.4, color="C0")
...
def _(mo):
    ...
    while keeping the parameter space manageable (4 variables x 2 lags x 4 equations =
...
@app.cell
def _(NUTSSampler, VAR, data, ic):
    sampler = NUTSSampler(draws=2500, tune=1000, chains=4, cores=1, random_seed=123)
    fitted_1 = VAR(lags=ic.bic, prior="minnesota").fit(data, sampler=sampler)
    fitted_1
    return (fitted_1,)
...
@app.cell
def _(Cholesky, fitted_1):
    identified = fitted_1.set_identification_strategy(
        Cholesky(ordering=["pressure", "wind", "temperature", "humidity"])
    )
    identified
    return (identified,)
